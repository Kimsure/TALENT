...
class Neck_CostAffinity(nn.Module):
    ...
    def forward(self, imgs, state):
        ...
        if self.training:
            b, c, h, w = fq3.shape
            fq3_flat = fq3.view(b, c, -1)  
            fq4_flat = fq4.view(b, c, -1)  
            fq5_flat = fq5.view(b, c, -1)  
            fq3_norm = F.normalize(fq3_flat, p=2, dim=1)  
            fq4_norm = F.normalize(fq4_flat, p=2, dim=1) 
            fq5_norm = F.normalize(fq5_flat, p=2, dim=1) 
            fq3_affinity = torch.einsum('bch,bcH->bhH', fq3_norm, fq3_norm)  
            fq4_affinity = torch.einsum('bch,bcH->bhH', fq4_norm, fq4_norm) 
            fq5_affinity = torch.einsum('bch,bcH->bhH', fq5_norm, fq5_norm)  
            fq3_affinity = F.softmax(fq3_affinity, dim=-1)  
            fq4_affinity = F.softmax(fq4_affinity, dim=-1)  
            fq5_affinity = F.softmax(fq5_affinity, dim=-1)  
            txt = txt.permute(1, 2, 0)
            B, D, L = txt.shape
            txt_norm = F.normalize(txt, p=2, dim=1) 
            fq3_txt_sim = torch.einsum('bcs,bcl->bsl', fq3_flat, txt)  
            fq4_txt_sim = torch.einsum('bcs,bcl->bsl', fq4_flat, txt) 
            fq5_txt_sim = torch.einsum('bcs,bcl->bsl', fq5_flat, txt)  
            fq3_lvaffinity = torch.einsum('bsl,bSl->bsS', fq3_txt_sim, fq3_txt_sim)
            fq4_lvaffinity = torch.einsum('bsl,bSl->bsS', fq4_txt_sim, fq4_txt_sim)  
            fq5_lvaffinity = torch.einsum('bsl,bSl->bsS', fq5_txt_sim, fq5_txt_sim)  
            fq3_lvaffinity = F.softmax(fq3_lvaffinity, dim=-1)
            fq4_lvaffinity = F.softmax(fq4_lvaffinity, dim=-1)
            fq5_lvaffinity = F.softmax(fq5_lvaffinity, dim=-1)
            fq3_lvaffinity = (fq3_lvaffinity > 0.7).float() 
            fq4_lvaffinity = (fq4_lvaffinity > 0.7).float() 
            fq5_lvaffinity = (fq5_lvaffinity > 0.7).float()  
