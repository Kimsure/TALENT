...
class CrossModalAttention(nn.Module):
    ...
    def forward(self, image_features, text_features, attention_mask=None, pad_mask=None):
        ...
        attn_output, _ = self.multihead_attn(query, key, value, attn_mask=attention_mask, key_padding_mask=pad_mask)
...
class CostVolume(nn.Module):
    def __init__(self, image_dim, text_dim, embed_dim):
        ...
        self.image_proj = nn.Linear(image_dim, embed_dim)
        self.text_proj = nn.Linear(text_dim, embed_dim)
        self.cost_proj = nn.Linear(17, embed_dim)
        self.back_proj = nn.Linear(embed_dim, image_dim)
    ...
    def forward(self, image_features, text_features, attention_mask=None, pad_mask=None):
        ...
        image_features = self.image_proj(image_features) # [B,L,D]
        text_features = self.text_proj(text_features)    # [B,K,D]
        query = image_features.permute(0, 2, 1) # [B,D,L]
        key = text_features
        cost_output = torch.einsum('bdl,bkd->blk', query, key) # [B,D,L] * [B,K,D] -> [B,L,K]
        # print("cost_output.shape", cost_output.shape)
        cost_output = self.cost_proj(cost_output)
        # cost_output = F.softmax(cost_output, dim=-1)  # Normalize along the last dimension
        cost_output = cost_output * image_features
        cost_output = self.back_proj(cost_output)
        
        return cost_output
    
    
class CostVolume2(nn.Module):
    def __init__(self, image_dim, text_dim, embed_dim):
        super(CostVolume2, self).__init__()    
        self.image_proj = nn.Linear(image_dim, embed_dim)
        self.text_proj = nn.Linear(text_dim, embed_dim)
        self.back_proj = nn.Linear(embed_dim, image_dim)
    ...
    def forward(self, image_features, text_features, attention_mask=None, pad_mask=None):
        ...
        key = text_features.permute(0, 2, 1) # [B,D,K]
        # print(query.shape, key.shape)
        cost_output = torch.einsum('bld,bdk->blk', query, key) # [B,L,D] * [B,D,K] -> [B,L,K]
        cost_output = F.softmax(cost_output, dim=-1)  # Normalize along the last dimension
        cost_output = torch.bmm(cost_output, key.permute(0, 2, 1))  # [B,L,K] * [B,K,D] -> [B,L,D]
        cost_output = self.back_proj(cost_output)
...
class DenseAligner(nn.Module):
    def __init__(
        ...
        self.cross = CostVolume(in_channels, text_dim, embed_dim)
        # self.cross = CostVolume2(in_channels, text_dim, embed_dim)
    ...
    def forward(self, x: Tensor, text_features,split_token=5, pad_mask=None) -> List[Tensor]:

        x0 = self.D_fc1(x)
        B,P,D = x0.shape
        W = H = int(math.sqrt(P-split_token))
        ...
        # text fusion
        outputs = self.cross(outputs, text_features, None, pad_mask)
...
class PromptAligner(nn.Module):
    ...
    def forward(self, x: Tensor, text_features) -> List[Tensor]:

        x0 = self.D_fc1(x)
        B,P,D = x0.shape
        x0 = F.relu(x0, inplace=True)
        # text fusion
        outputs = self.cross(x0, text_features, None, None)
        outputs += x0
        outputs = self.D_fc2(outputs)
        if self.skip_connect:
            outputs+=x
