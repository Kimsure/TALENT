...
# from utils.dataset_nemo import tokenize
...
def inference(test_loader, model, args):
    ...
    for img, param in tbar:
        ...
        for sent in param['sents']:
            ...
            # 找到小于0.2的iou, 将其置为其他的均值
            min_index = tmp_iou_list.index(min(tmp_iou_list))
            # if tmp_iou_list[min_index] < 0.05:
            #     # 排除最小值后再计算均值
            #     # filtered_iou = [x for i, x in enumerate(tmp_iou_list) if i != min_index]
            #     replacement_value = np.mean(tmp_iou_list) if tmp_iou_list else 0  # 处理全小于0.2的情况
            #     tmp_iou_list[min_index] = replacement_value
            ...
            if args.visualize:
                ...
                cv2.imwrite(filename=os.path.join(args.vis_dir, pred_name),
                            img=pred)
                # draw the pred overlay on the image
                # 创建预测结果的彩色遮罩
                pred_color = np.zeros_like(param['ori_img'][0].cpu().numpy())
                pred_color[pred == 255] = [255, 0, 0]  # 使用红色表示预测区域
                # 将预测遮罩与原图混合
                overlay = cv2.addWeighted(param['ori_img'][0].cpu().numpy(), 
                                        0.6,  # 原图权重 
                                        pred_color,
                                        0.6,  # 遮罩权重
                                        0)
                # 保存叠加后的图像
                overlay_name = '{}-overlay-{}.png'.format(seg_id, sent)
                cv2.imwrite(filename=os.path.join(args.vis_dir, overlay_name),
                           img=overlay)
                    

        iou_list.extend(tmp_iou_list)
